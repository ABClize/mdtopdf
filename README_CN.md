...
默认主题现在使用 PDFium 更安全的 Latin-first 字体栈。Latin 字体会排在 CJK
字体前面，避免英文、数字、日期、版本号和页码被 WeasyPrint 放进 CJK 字体子集，
从而在 Chrome/PDFium 里出现数字不可见的问题。中文仍然会回退到 CJK 字体栈；
在 CJK fallback 里，`Microsoft YaHei` 仍是第一优先级。如果 Linux 环境里没装
微软雅黑，才会继续回退到 `PingFang SC`、`Noto Sans SC`、`Noto Sans CJK SC`、
...
这只是兜底组合。如果你希望 Linux 的中文排版尽量接近 Windows 默认效果，需要在
运行环境里自行合法提供 `Microsoft YaHei`。`mdtopdf` 只在 CSS 里引用这个字体名，
不会打包、下载或分发微软字体文件。提供微软雅黑只影响中文 fallback；Latin-first
顺序仍然会保留，用来保证 PDFium 下的数字更稳定。

代码块会优先使用 `Cascadia Mono` / `Cascadia Code`，然后回退到 `Consolas`、
`Noto Sans Mono CJK SC`、`Liberation Mono`、`DejaVu Sans Mono`。Debian
可以用 `fonts-cascadia-code` 安装 Cascadia 字体；其他 Linux 镜像如果没有这个包，
就自行把 Cascadia Code 放进运行环境，或者接受主题回退到已有等宽字体。
