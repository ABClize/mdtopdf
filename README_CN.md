...
这套组合下，Linux 的英文和数字走 `Liberation Sans` / `DejaVu Sans`，中文正文走
`Noto Sans CJK SC`，数学 fallback 走 STIX。`Microsoft YaHei` 和
`Segoe UI Emoji` 不是 Linux 依赖，`mdtopdf` 不会安装、下载或分发这些微软字体。
...
Emoji 走系统 emoji 字体。Linux 上更推荐单色 `Noto Emoji`，PDF 版面通常比彩色
